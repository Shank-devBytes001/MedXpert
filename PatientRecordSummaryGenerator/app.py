...
def chat():
    ...
    try:
        ...
        app.logger.info(f"Chat interaction - Question: {message}, API Response: {api_response}")

        # Extract the actual chat response text from the API response
        chat_response_text = "Could not parse chat response."
        if isinstance(api_response, dict) and 'response' in api_response:
             chat_response_text = api_response['response']
        elif isinstance(api_response, str):
             chat_response_text = api_response
